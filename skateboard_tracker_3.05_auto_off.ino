...
void loop()
{
  ...
  if (newData) {
    ...
    if (first) {
      ...
      if (screenOn) {
        ...
        if (sp <= 6) { //wheels down sitting around
          countOff++;
        } else {
          countOff = 0;
        }
  ...
  previousIdle = currentMillis;
  } else {

    if (currentMillis - previousIdle >= interval) { //30min
      saveSpeed();
      saveDist();
      digitalWrite(onPin, LOW);
    }
  ...
  display.display();
  display.clearDisplay();
  if (AcZ * 9.8 < -7 && screenOn) {
    display.clearDisplay();
    display.display();
    screenOn = false;
  } else if (!screenOn && AcZ * 9.8 >= -7) {
    screenOn = true;
  }
 ...
 // prev = currentMillis;
  ...
  if (currentMillis - previous >= 600000 && (AcZ * 9.8 >= -7.0)) { //10min
    saveSpeed();
    saveDist();
    digitalWrite(onPin, LOW);
  }
